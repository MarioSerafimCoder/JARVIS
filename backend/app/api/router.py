...
from app.container import agent, provider, settings, tool_executor, tool_registry
...
async def chat(payload: ChatInput) -> dict:
    try:
        ...
        raise HTTPException(status_code=503, detail=f"Falha ao conversar com o modelo local: {exc}") from exc
...
def rename_conversation(conversation_id: str, payload: dict[str, str]) -> dict:
    ...
    return repository.row("SELECT * FROM conversations WHERE id=?", (conversation_id,)) or {}
...
def pending_tools() -> list[dict]:
    items = repository.rows("SELECT * FROM pending_actions WHERE status='pending' ORDER BY created_at DESC")
    for item in items:
        item["input"] = json.loads(item.pop("input_json"))
    return items


@router.post("/tools/{action_id}/confirm")
def confirm_tool(action_id: str, payload: ConfirmationInput) -> dict:
    try:
        return tool_executor.confirm(action_id, payload.approved)
    except ValueError as exc:
