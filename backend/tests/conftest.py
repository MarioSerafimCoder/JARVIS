...
def isolated_data(tmp_path: Path):
    ...
    previous = (settings.database_path, settings.library_path, settings.notes_path, settings.backup_path)
    ...
    initialize_database()
    yield settings
    settings.database_path, settings.library_path, settings.notes_path, settings.backup_path = previous
