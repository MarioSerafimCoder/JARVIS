...
const STATE_COLOR: Record<CognitiveState, number> = { IDLE:0xc78645, THINKING:0xffa62b, SEARCHING_MEMORY:0xffbf58, SEARCHING_KNOWLEDGE:0xffd894, USING_TOOL:0xff7628, WAITING_CONFIRMATION:0xeaa54d, ERROR:0xff4d3d, LISTENING:0xc78645, SPEAKING:0xc78645 }
export function CognitiveScene({ graph, state, highlighted, selectedId, onSelect, quality, resetKey, forceFallback = false }: { graph: CognitiveGraph; state: CognitiveState; highlighted: Set<string>; selectedId?: string; onSelect: (node?: CognitiveNode) => void; quality: CognitiveQuality; resetKey: number; forceFallback?: boolean }) {
  ...
  useEffect(() => {
    ...
    scene.add(new THREE.AmbientLight(0x8c542c, .5)); const coreLight = new THREE.PointLight(STATE_COLOR[state], 14, 22, 2); scene.add(coreLight)

    const coreMaterial = new THREE.MeshStandardMaterial({color:STATE_COLOR[state],emissive:STATE_COLOR[state],emissiveIntensity:.72,roughness:.34,metalness:.45,wireframe:true})
    ...
    renderNodes.forEach((node,index)=>{dummy.position.set(node.position.x,node.position.y,node.position.z);const boost=highlighted.has(node.id)?1.65:selectedId===node.id?1.38:1;dummy.scale.setScalar(node.size*boost);dummy.updateMatrix();instances.setMatrixAt(index,dummy.matrix);color.setHex(COLORS[node.cluster]||COLORS.other);if(highlighted.has(node.id)||selectedId===node.id)color.lerp(new THREE.Color(0xffe0a3),.58);instances.setColorAt(index,color)})
    ...
    for(const edge of sceneGraph.edges){const a=byId.get(edge.source),b=byId.get(edge.target);if(!a||!b)continue;edgePositions.push(a.position.x,a.position.y,a.position.z,b.position.x,b.position.y,b.position.z);const alpha=.16+edge.weight*.32;edgeColors.push(.95*alpha,.48*alpha,.16*alpha,.95*alpha,.48*alpha,.16*alpha)}
    ...
    const focusNode=selectedId?byId.get(selectedId):undefined;const desiredTarget=focusNode?new THREE.Vector3(focusNode.position.x,focusNode.position.y,focusNode.position.z):undefined;const desiredCamera=desiredTarget?.clone().add(new THREE.Vector3(7,5,11))
    let frame=0,raf=0,last=performance.now(),fpsStart=last
    const animate=(now:number)=>{frame++;const elapsed=(now-last)/1000;last=now;const active=state!=='IDLE';core.rotation.y+=elapsed*(active ? .58 : .16);core.rotation.x+=elapsed*.08;const pulse=1+Math.sin(now*(active ? .004 : .0018))*(active ? .07 : .025);core.scale.setScalar(pulse);coreLight.intensity=(active ? 16 : 10)*(1+Math.sin(now*.003)*.08);points.rotation.y+=elapsed*.018;if(desiredTarget&&desiredCamera){controls.target.lerp(desiredTarget,.055);camera.position.lerp(desiredCamera,.045)}controls.update();renderer.render(scene,camera);if(quality==='AUTO'&&now-fpsStart>2500){const fps=frame/((now-fpsStart)/1000);if(fps<27)renderer.setPixelRatio(.75);frame=0;fpsStart=now}raf=requestAnimationFrame(animate)};raf=requestAnimationFrame(animate)
    return()=>{cancelAnimationFrame(raf);observer.disconnect();renderer.domElement.removeEventListener('pointerup',click);controls.dispose();sphere.dispose();nodeMaterial.dispose();linesGeometry.dispose();pointsGeometry.dispose();core.geometry.dispose();coreMaterial.dispose();renderer.dispose();renderer.domElement.remove()}
  },[sceneGraph,state,highlighted,selectedId,onSelect,quality,resetKey,forceFallback])
...
export function CognitiveFallback({graph,highlighted,selectedId,onSelect}:{graph:CognitiveGraph;highlighted:Set<string>;selectedId?:string;onSelect:(node?:CognitiveNode)=>void}){
  ...
  return <svg className="cognitive-fallback" viewBox="0 0 100 100" role="img" aria-label="Mapa bidimensional acessível da memória viva do Jarvis">{graph.edges.map((edge,index)=>{const a=byId.get(edge.source),b=byId.get(edge.target);if(!a||!b)return null;const pa=point(a),pb=point(b);return <line key={index} x1={pa.x} y1={pa.y} x2={pb.x} y2={pb.y} opacity={.18+edge.weight*.4}/>})}{graph.nodes.map(node=>{const p=point(node);return <circle key={node.id} cx={p.x} cy={p.y} r={node.kind==='core'?2.8:Math.max(.7,node.size)} className={`${node.kind} ${highlighted.has(node.id)?'highlighted':''} ${selectedId===node.id?'selected':''}`} onClick={()=>onSelect(node)}><title>{node.label}</title></circle>})}</svg>
}
