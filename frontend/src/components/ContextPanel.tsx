...
export function ContextPanel({ context, close }: { context: ContextEvidence; close: () => void }) {
  const memories = context.memories || [], documents = context.documents || [], tasks = context.tasks || [], actions = context.actions || [], sources=context.web?.sources||[], pages=context.web?.pages||[], browser=context.browser||[]
  return <aside className="context-panel"><div className="panel-head"><div><span className="eyebrow">EVIDÊNCIAS</span><strong>Inspetor de contexto</strong></div><button aria-label="Fechar contexto" className="icon-button" onClick={close}><X/></button></div>
    ...
    <Section title="Web" count={sources.length}>{sources.length?<>{context.web?.queries.map((item,index)=><article key={`q-${index}`}><strong>Consulta sanitizada</strong><p>{item.query}</p>{item.redactions?.length?<small>Dados removidos: {item.redactions.join(', ')}</small>:null}</article>)}{sources.map(item=><article key={item.source_id}><a href={item.url} target="_blank" rel="noreferrer"><strong>{item.title}</strong></a><small>{item.domain} · recuperado {new Date(item.retrieved_at).toLocaleString('pt-BR')}</small><p>{item.excerpt}</p></article>)}{pages.length>0&&<small>{pages.length} página(s) lida(s); conteúdo tratado como não confiável.</small>}</>:<small>Nenhuma fonte web utilizada.</small>}</Section>
    <Section title="Navegador" count={browser.length}>{browser.length?browser.map((item,index)=><article key={`${item.action}-${index}`}><strong>{item.action.replaceAll('_',' ')}</strong><small>{item.site} · {item.verified?'verificado':'não verificado'} · {item.status}</small></article>):<small>Nenhuma ação de navegador.</small>}</Section>
